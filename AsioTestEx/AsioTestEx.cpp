...
struct ClientContext : public boost::enable_shared_from_this < ClientContext >
{
	...
	void OnSend(const boost::system::error_code & ec, std::list< std::vector< boost::uint8_t > >::iterator itr)
	{
		...
		{
			global_stream_lock.lock();
			std::cout << "[" << boost::this_thread::get_id() << "] Sent " << (*itr).size() << " bytes." << std::endl;
			global_stream_lock.unlock();
		}
		...
		if (!m_send_buffer.empty())
		{
			boost::asio::async_write(
				...
				boost::bind(&ClientContext::OnSend, shared_from_this(), boost::asio::placeholders::error, m_send_buffer.begin())
			);
		}
	}
	...
	void OnRecv(const boost::system::error_code & ec, size_t bytes_transferred)
	{
		...
		else
		{
			...
			// Debug information
			global_stream_lock.lock();
			std::cout << "[" << boost::this_thread::get_id() << "] Recv " << bytes_transferred << " bytes." << std::endl;
			global_stream_lock.unlock();
	...
	void Recv()
	{
		m_socket.async_read_some(
			boost::asio::buffer(&m_recv_buffer[m_recv_buffer_index],m_recv_buffer.size() - m_recv_buffer_index),
...
int main(int argc, char * argv[])
{
	...
	boost::shared_ptr< ClientContext > client( new ClientContext(*io_service) );
